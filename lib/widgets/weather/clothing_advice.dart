...
class ClothingAdviceCard extends StatelessWidget {
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              child: Icon(advice.icon, color: accent, size: 24),
            ),
  ...
  _Advice _generateAdvice(InsightTextStyle style) {
    ...
    final next6h = weather.hourly.where(
      (h) =>
          h.time.isAfter(now) &&
          h.time.isBefore(now.add(const Duration(hours: 6))),
    );
    ...
    if (feelsLike < 5) {
      final coolNote = willCoolDown
          ? _t(
              style,
              ' Thermal drop imminent.',
              ' Getting colder later!',
              ' Getting colder later.',
            )
          : '';
      return _Advice(
    ...
    if (feelsLike < 15) {
      final windNote = wind > 20
          ? _t(
              style,
              ' Wind chill active — add layer.',
              ' A bit breezy too, so layer up!',
              ' It\'s breezy, so layer up.',
            )
          : '';
      return _Advice(
    ...
    if (feelsLike > 30) {
      ...
      String kawaiiDetail =
          'Light clothing and breathable fabrics will feel best!';
